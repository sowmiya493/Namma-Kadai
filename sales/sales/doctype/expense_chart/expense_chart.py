...
class ExpenseChart(Document):
    ...
    def _generate_chart_html(self, expense_type, data, max_amount, y_axis_ticks):
        ...
        <div class="x-axis" style="display: flex; justify-content: space-between; margin-top: 10px; padding-left: 50px;">
        '''
        for record in data:
            chart_html += f'''
            <div style="text-align: center; width: 10%; font-size: 10px;">{record['user']}</div>
            '''
        chart_html += '''
        </div>
        </div>
